...
import { PADDING, COLORS, getGraph, fetchGraphData, GraphData } from "./Model";
...
export const Graph = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        setError(null);
        const data = await fetchGraphData("SOLUSDT");
        setGraphData(data);
      } catch (err) {
  ...
  const graphs = useMemo(() => {
    if (!graphData) return [];
    return getGraph(graphWidth, height, graphData);
  }, [graphWidth, height, graphData]);
  ...
  if (loading) {
    return (
      <View style={{backgroundColor: cardBg, borderRadius: 20, overflow: 'hidden', height: height + 200, justifyContent: 'center', alignItems: 'center'}}>
        <Text style={{color: '#666', fontSize: 16}}>Loading chart data...</Text>
      </View>
    );
  }
  ...
  if (error || graphs.length === 0) {
    return (
      <View style={{backgroundColor: cardBg, borderRadius: 20, overflow: 'hidden', height: height + 200, justifyContent: 'center', alignItems: 'center'}}>
        <Text style={{color: '#ff6b6b', fontSize: 16}}>{error || 'No data available'}</Text>
      </View>
    );
  }
